...
SWHTable <- cbind(TotalData$SWH, TotalData$ab_m3_Littorina_saxatilis)
...
jpeg(file = paste(SaveHere, "SWHab_m3_Littorina_saxatilis.jpg"), height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Significant Wave Height (m)", ylab= expression ("Abundance (Ind/m"^3*")"), main=" ",ylim=c(0,500))              # plots the barplot and saves the midpoints in mp
