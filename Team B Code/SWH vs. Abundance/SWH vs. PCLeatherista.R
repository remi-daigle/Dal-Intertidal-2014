...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(pc_Leatherista_difformus/Quadrat_m2,na.rm = TRUE)*100,
                    SDAbundance=sd(pc_Leatherista_difformus/Quadrat_m2,na.rm = TRUE)*100
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$pc_Leatherista_difformus)
...
jpeg(file = paste(SaveHere, "SWHpc_Leatherista_difformus.jpg"), height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Significant Wave Height (m)", ylab= expression ("Percent Cover (%)"), main=" ",ylim=c(0,10))              # plots the barplot and saves the midpoints in mp
