...
SWHTable <- cbind(TotalData$SWH, TotalData$ab_m3_Semibalanus_balanoides)
...
jpeg(file = paste(SaveHere, "SWHab_m3_Semibalanus_balanoides.jpg"), height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Significant Wave Height (m)", ylab= expression ("Abundance (Ind/m"^3*")"), main=" ",ylim=c(0,5000))              # plots the barplot and saves the midpoints in mp
