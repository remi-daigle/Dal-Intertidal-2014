...
SWHTable <- cbind(TotalData$SWH, TotalData$ab_m2_Apohyale_prevostii)
...
jpeg(file = paste(SaveHere, "SWHab_m2_Apohyale_prevostii.jpg"), height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Significant Wave Height (m)", ylab= expression ("Abundance (Ind/m"^2*")"), main=" ",ylim=c(0,1))              # plots the barplot and saves the midpoints in mp
