...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(pc_Fucus_distichous_attached/Quadrat_m2,na.rm = TRUE)*100,
                    SDAbundance=sd(pc_Fucus_distichous_attached/Quadrat_m2,na.rm = TRUE)*100
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$pc_Fucus_distichous_attached)
...
jpeg(file = paste(SaveHere, "SWHpc_Fucus_distichous_attached.jpg"), height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Significant Wave Height (m)", ylab= expression ("Percent Cover (%)"), main=" ",ylim=c(0,100))              # plots the barplot and saves the midpoints in mp
