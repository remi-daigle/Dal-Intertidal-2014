...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(ab_m2_Pagurus_longicarpus/Quadrat_m2,na.rm = TRUE),
                    SDAbundance=sd(ab_m2_Pagurus_longicarpus/Quadrat_m2,na.rm = TRUE),
                    NAbundance=sum(is.na(ab_m2_Pagurus_longicarpus)==F)
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$ab_m2_Pagurus_longicarpus)
...
jpeg('SWHAbundancePagarusM2', height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Standard Wave Height", ylab= expression ("Abundance (Ind/m"^2*")"), main=" ",ylim=c(0,200))              # plots the barplot and saves the midpoints in mp
