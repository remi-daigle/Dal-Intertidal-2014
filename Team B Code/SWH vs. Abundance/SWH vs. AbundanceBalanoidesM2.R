...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    ...
                    SDAbundance=sd(ab_m2_Semibalanus_balanoides/Quadrat_m2,na.rm = TRUE)
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$ab_m2_Semibalanus_balanoides)
...
jpeg(file = paste(SaveHere, "SWHAbundanceab_m2_Semibalanus_balanoides.jpg"), height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Significant Wave Height (m)", ylab= expression ("Abundance (Ind/m"^2*")"), main=" ",ylim=c(0,1000))              # plots the barplot and saves the midpoints in mp
