...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(pc_Acrosiphonia_arcta/Quadrat_m2,na.rm = TRUE)*100,
                    SDAbundance=sd(pc_Acrosiphonia_arcta/Quadrat_m2,na.rm = TRUE)*100
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$pc_Acrosiphonia_arcta)
...
jpeg(file = paste(SaveHere, "SWHpc_Acrosiphonia_arcta.jpg"), height=1200, width=2400, res=400, qual=100  )
