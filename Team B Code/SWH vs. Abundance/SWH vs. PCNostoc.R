...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(pc_Nostoc_cyanobacteria/Quadrat_m2,na.rm = TRUE)*100,
                    SDAbundance=sd(pc_Nostoc_cyanobacteria/Quadrat_m2,na.rm = TRUE)*100
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$pc_Nostoc_cyanobacteria)
...
jpeg(file = paste(SaveHere, "SWHpc_Nostoc_cyanobacteria.jpg"), height=1200, width=2400, res=400, qual=100  )
