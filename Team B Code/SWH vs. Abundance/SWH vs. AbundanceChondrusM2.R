...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(ab_m2_Chondrus.crispus/Quadrat_m2,na.rm = TRUE),
                    SDAbundance=sd(ab_m2_Chondrus.crispus/Quadrat_m2,na.rm = TRUE),
                    NAbundance=sum(is.na(ab_m2_Chondrus.crispus)==F)
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$ab_m2_Chondrus.crispus)
...
jpeg('SWHAbundanceChondrusM2', height=1200, width=2400, res=400, qual=100  )
