...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(ab_m3_Pagurus_longicarpus/Core_m3,na.rm = TRUE),
                    SDAbundance=sd(ab_m3_Pagurus_longicarpus/Core_m3,na.rm = TRUE)
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$ab_m3_Pagurus_longicarpus)
...
jpeg(file = paste(SaveHere, "SWHab_m3_Pagurus_longicarpus.jpg"), height=1200, width=2400, res=400, qual=100  )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SWH, xlab="Significant Wave Height (m)", ylab= expression ("Abundance (Ind/m"^3*")"), main=" ",ylim=c(0,50))              # plots the barplot and saves the midpoints in mp
