...
FigureTable<- ddply(TotalData,.(SWH),summarize,
                    MeanAbundance=mean(Pc_ascophyllum.nodosum/Quadrat_m2,na.rm = TRUE)*100,
                    SDAbundance=sd(Pc_ascophyllum.nodosum/Quadrat_m2,na.rm = TRUE)*100
)

### Make SWH Table ####

#Bind salinity and abundance in data table
SWHTable <- cbind(TotalData$SWH, TotalData$Pc_ascophyllum.nodosum)
...
jpeg(file = paste(SaveHere, "SWHPc_ascophyllum.nodosum.jpg"), height=1200, width=2400, res=400, qual=100  )
