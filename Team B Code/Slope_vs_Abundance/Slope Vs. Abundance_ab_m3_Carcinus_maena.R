...
FigureTableSlope<- ddply(TotalData,.(Slope),summarize,
                       MeanAbundance=mean(ab_m3_Carcinus_maena/Core_m3,na.rm = TRUE),
                       NAbundance=sum(is.na(ab_m3_Carcinus_maena)==F)
)
...
jpeg('SLOPEAbundanceab_m3_Carcinus_maena', height=1200, width=2400, res=400, qual=100)
