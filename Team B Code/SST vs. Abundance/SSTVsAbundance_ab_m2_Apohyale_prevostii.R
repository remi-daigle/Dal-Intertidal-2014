...
SSTTable <- cbind(TotalData$SST, TotalData$ab_m2_Apohyale_prevostii)
...
FigureTable<- ddply(TotalData,.(SST),summarize,
                    MeanAbundance=mean(ab_m2_Apohyale_prevostii/Quadrat_m2,na.rm = TRUE),
                    SDAbundance=sd(ab_m2_Apohyale_prevostii/Quadrat_m2,na.rm = TRUE),
                    NAbundance=sum(is.na(ab_m2_Apohyale_prevostii)==F)
)
###### Make Graph ########
jpeg('SSTAbundanceab_m2_Apohyale_prevostii.jpeg', height=1200, width=2400, res=400, qual=100 )
...
jpeg('SSTAbundanceab_m2_Apohyale_prevostii.jpeg', height=1200, width=2400, res=400, qual=100 )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SST, xlab="SST", ylab= expression ("Abundance (Ind/m"^2*")"), main=" ",ylim=c(0,50)) # plots the barplot and saves the midpoints in mp
