...
SSTTable <- cbind(TotalData$SST, TotalData$ab_m3_Chiridotea_coeca)
...
FigureTable<- ddply(TotalData,.(SST),summarize,
                    MeanAbundance=mean(ab_m3_Chiridotea_coeca/Core_m3,na.rm = TRUE),
                    SDAbundance=sd(ab_m3_Chiridotea_coeca/Core_m3,na.rm = TRUE),
                    NAbundance=sum(is.na(ab_m3_Chiridotea_coeca)==F)
)

###### Make Graph ########

jpeg('SSTAbundanceab_m3_Chiridotea_coeca.jpeg', height=1200, width=2400, res=400, qual=100 )
...
jpeg('SSTAbundanceab_m3_Chiridotea_coeca.jpeg', height=1200, width=2400, res=400, qual=100 )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SST, xlab="SST", ylab= expression ("Abundance (Ind/m"^3*")"), main=" ",ylim=c(0,100)) # plots the barplot and saves the midpoints in mp
