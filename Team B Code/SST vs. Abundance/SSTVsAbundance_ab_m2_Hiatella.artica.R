...
SSTTable <- cbind(TotalData$SST, TotalData$ab_m2_Hiatella.artica)
...
FigureTable<- ddply(TotalData,.(SST),summarize,
                    MeanAbundance=mean(ab_m2_Hiatella.artica/Quadrat_m2,na.rm = TRUE),
                    SDAbundance=sd(ab_m2_Hiatella.artica/Quadrat_m2,na.rm = TRUE),
                    NAbundance=sum(is.na(ab_m2_Hiatella.artica)==F)
)
###### Make Graph ########
jpeg('SSTAbundanceab_m2_Hiatella.artica.jpeg', height=1200, width=2400, res=400, qual=100 )
