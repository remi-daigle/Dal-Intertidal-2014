...
SSTTable <- cbind(TotalData$SST, TotalData$ab_m3_Glycera_dibrachiata)
...
FigureTable<- ddply(TotalData,.(SST),summarize,
                    MeanAbundance=mean(ab_m3_Glycera_dibrachiata,na.rm = TRUE),
...
jpeg('SSTAbundanceab_m3_Glycera_dibrachiata.jpeg', height=1200, width=2400, res=400, qual=100 )
...
jpeg('SSTAbundanceab_m3_Glycera_dibrachiata.jpeg', height=1200, width=2400, res=400, qual=100 )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SST, xlab="SST", ylab= expression ("Abundance (Ind/m"^3*")"), main=" ",ylim=c(0,50)) # plots the barplot and saves the midpoints in mp
