...
SSTTable <- cbind(TotalData$SST, TotalData$ab_m3_Asterias.spp.)
...
FigureTable<- ddply(TotalData,.(SST),summarize,
                    MeanAbundance=mean(ab_m3_Asterias.spp.,na.rm = TRUE),
                    SDAbundance=sd(ab_m3_Asterias.spp./Core_m3,na.rm = TRUE),
                    NAbundance=sum(is.na(ab_m3_Asterias.spp.)==F)
)

###### Make Graph ########

jpeg('SSTAbundanceab_m3_Asterias.spp..jpeg', height=1200, width=2400, res=400, qual=100 )
...
jpeg('SSTAbundanceab_m3_Asterias.spp..jpeg', height=1200, width=2400, res=400, qual=100 )
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$SST, xlab="SST", ylab= expression ("Abundance (Ind/m"^3*")"), main=" ",ylim=c(0,1)) # plots the barplot and saves the midpoints in mp
