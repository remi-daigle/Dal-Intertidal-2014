...
SSTTable <- cbind(TotalData$Salinity, TotalData$Pc_ascophyllum.nodosum)
...
FigureTable<- ddply(TotalData,.(Salinity),summarize,
                    MeanPercentCoverage=mean(Pc_ascophyllum.nodosum,na.rm = TRUE),
                    SDPercentCoverage=sd(Pc_ascophyllum.nodosum,na.rm = TRUE),
                    NPercentCoverage=sum(is.na(Pc_ascophyllum.nodosum)==F)
)
...
mp <- barplot(AB_mean, names.arg=FigureTable$Salinity, xlab="Salinity", ylab= expression ("Percent coverage (%)"), main=" ",ylim=c(0,15)) # plots the barplot and saves the midpoints in mp
