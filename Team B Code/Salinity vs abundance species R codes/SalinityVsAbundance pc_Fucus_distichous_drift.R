...
SalinityTable <- cbind(TotalData$Salinity, TotalData$pc_Fucus_distichous_drift)
...
FigureTable<- ddply(TotalData,.(Salinity),summarize,
                MeanAbundance=mean(pc_Fucus_distichous_drift/Quadrat_m2,na.rm = TRUE),
                SDAbundance=sd(pc_Fucus_distichous_drift/Quadrat_m2,na.rm = TRUE),
                NAbundance=sum(is.na(pc_Fucus_distichous_drift)==F)
)

####change y - axis to m cubed when have to for species...check!
###### Make Graph ########
jpeg('SalinityAbundance1.jpeg', height=1200, width=2400, res=400, qual=100  )
barplot(FigureTable$MeanAbundance, names.arg=FigureTable$Salinity, xlab="Salinity", ylab= expression ("Percent Cover (%) (Ind/m"^2*")"), main=" ")
dev.off()
...
mp <- barplot(FigureTable$MeanAbundance, names.arg=FigureTable$Salinity, xlab="Salinity", ylab= expression ("Percent Cover (%) (Ind/m"^2*")"), main=" ",ylim=c(0,250))              # plots the barplot and saves the midpoints in mp
