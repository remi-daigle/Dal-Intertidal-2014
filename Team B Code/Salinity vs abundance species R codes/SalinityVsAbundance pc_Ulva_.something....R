...
SSTTable <- cbind(TotalData$Salinity, TotalData$pc_Ulva_.something...)
...
FigureTable<- ddply(TotalData,.(Salinity),summarize,
                    MeanPercentCoverage=mean(pc_Ulva_.something...,na.rm = TRUE),
                    SDPercentCoverage=sd(pc_Ulva_.something...,na.rm = TRUE),
                    NPercentCoverage=sum(is.na(pc_Ulva_.something...)==F)
)
...
mp <- barplot(AB_mean, names.arg=FigureTable$Salinity, xlab="Salinity", ylab= expression ("Percent coverage (%)"), main=" ",ylim=c(0,1)) # plots the barplot and saves the midpoints in mp
